...
| 기능 | 상태 |
|---|---|
| k3s + Traefik 배포, HTTPS, CloudNativePG | **출시** — 앱 파드와 마이그레이션 initContainer 모두 PR #48의 애플리케이션 커밋을 담은 `ghcr.io/cgm-16/clip:sha-7109e3d`. 이후 `main`의 변경은 문서 전용 PR #49입니다 |
...
| 새 길드 권한 매트릭스 실측, 통합 경쟁 테스트 패스, 17개 수동 시나리오 | **부분 완료** — `6.1`은 부트스트랩 권한 회수 경계를 실측했고, `6.3`은 실행 가능한 14개 시나리오가 통과했습니다. 범위에서 잘린 3개는 실행하지 않았고, `6.2` 통합 패스는 이 기록에서 완료로 주장하지 않습니다 |
...
**`6.1`과 `6.3`은 배포 뒤 실제 길드에서 부분적으로 닫혔고, `6.2` 통합 패스는 이 기록에서 완료로 주장하지 않습니다.** `6.3`의 17개 중 출시된 표면에 해당하는 14개는 통과했고, 역할 설정 UI와 웹 아카이브에 의존하는 3개는 이미 승인된 범위 컷이라 실행하지 않았습니다. `6.1`은 자동 생성 뒤 `MANAGE_CHANNELS`를 회수해도 런타임 Clip/Unclip이 동작함을 확인했지만, 모든 권한을 하나씩 회수한 완전한 최소 매트릭스는 아닙니다.
...
- **실제 길드에서 수동 E2E를 수행했습니다.** `6.3`의 실행 가능한 14개 시나리오가 통과했습니다. 설정의 새 채널/기존 채널 경로, 권한 거부, 중복 Clip, 두 사용자 보존 신호, 부분/최종 Unclip, 작성자 삭제와 툼스톤, 원본 수정/삭제 뒤 스냅샷, 차단된 DM, 만료 세션을 사람이 Discord와 웹 화면에서 확인했습니다. 나머지 3개는 역할 설정 UI 또는 웹 아카이브에 의존해 승인된 범위 컷과 함께 실행 대상에서 빠졌습니다.
- **브라우저 E2E가 없습니다.** Playwright(`6.4`)는 잘렸습니다. 설정 화면은 jsdom 위 React Testing Library로 덮여 있습니다.
- **완전한 최소 권한 매트릭스는 없습니다.** `MANAGE_CHANNELS`를 부트스트랩 뒤 회수하고 Clip/Unclip을 다시 통과시켜 런타임에는 필요 없음을 실측했습니다. 다른 런타임 권한은 묶음으로 검증했으며 하나씩 회수하지 않았습니다(`6.1`).
- **`6.2` 통합 경쟁 테스트 패스는 이 기록에서 완료로 주장하지 않습니다.** 실제 PostgreSQL 단위 테스트의 경쟁·멱등성 커버리지는 그대로지만, 별도 통합 패스를 수동 POC 결과와 혼동하지 않습니다.
...
Clip은 `Administrator`를 요구하지 않습니다. 실제 애플리케이션에서 봇이 `ADMINISTRATOR`가 아님을 확인했고, 자동 생성이 끝난 뒤 `MANAGE_CHANNELS`도 회수했습니다. 그 상태에서 새 Clip과 Unclip이 모두 성공했습니다.

| 권한 또는 애플리케이션 설정 | 왜 | 언제 |
|---|---|---|
| Message Content Intent | 원본 본문을 읽어 전달 스냅샷을 만들기 위해 필요. 비활성 상태에서는 메시지 조회가 성공해도 본문이 가려지고 전달이 에러 `160014`로 거부됨 | 상시 |
| 원본 채널의 `VIEW_CHANNEL` + `READ_MESSAGE_HISTORY` | 선택한 원본 메시지 조회 | 상시 |
...
이 표는 **관측된 운영 전제**이지 완전한 최소 권한 증명은 아닙니다. `MANAGE_CHANNELS`의 부트스트랩 한정 경계와 Message Content Intent 실패 원인은 실제 길드에서 확인했습니다. 나머지 런타임 권한은 성공한 구성의 일부였지만 하나씩 회수해 반증하지 않았습니다.
...
- **기존 채널 설정 경로는 봇의 실효 권한을 저장 전에 검증하지 않습니다.** POC의 기존 채널 시나리오는 봇이 이미 읽고 쓸 수 있는 `#clip-archive`에서 통과했습니다. 관리자는 기존 채널을 고를 때 봇의 `VIEW_CHANNEL`과 `SEND_MESSAGES`를 직접 확인해야 합니다. 마감 직전 핫픽스 대신 알려진 P0 운영 제약으로 남겼습니다.
